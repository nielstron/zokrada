...
def pairing_addition(p1: G1Point, p2: G1Point) -> G1Point:
    ...
    from groth16.bn128.bn128_curve import add, FQ2, FQ
    mp1 = (FQ(p1.x), FQ(p1.y))
    mp2 = (FQ(p2.x), FQ(p2.y))
    mr = add(mp1, mp2)
...
def pairing_scalar_mul(p: G1Point, s: int) -> G1Point:
    ...
    from groth16.bn128.bn128_curve import multiply, FQ
    mp = (FQ(p.x), FQ(p.y))
    mr = multiply(mp, s)
    r = G1Point(mr[0].n, mr[1].n)
    return r
...
def pairing_check(ps1: List[G1Point], ps2: List[G2Point]) -> bool:
    ...
    mp2 = [(FQ2([FQ(p2.x1), FQ(p2.y1)]), FQ2([FQ(p2.x2), FQ(p2.y2)])) for p2 in ps2]
